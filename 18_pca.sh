...
grep "#" cossypha_pca_25.recode.vcf > cossypha.vcf

for i in $( ls cossypha_pca_*.recode.vcf ); do grep -v "^#" $i >> cossypha.vcf; done


grep "#" cristrio_pca_25.recode.vcf > cristrio.vcf

for i in $( ls cristrio_pca_*.recode.vcf ); do grep -v "^#" $i >> cristrio.vcf; done


grep "#" critrist_pca_25.recode.vcf > critrist.vcf

for i in $( ls critrist_pca_*.recode.vcf ); do grep -v "^#" $i >> critrist.vcf; done


grep "#" melaenornis_pca_25.recode.vcf > melaenornis.vcf

for i in $( ls melaenornis_pca_*.recode.vcf ); do grep -v "^#" $i >> melaenornis.vcf; done


grep "#" nectarinia_pca_25.recode.vcf > nectarinia.vcf

for i in $( ls nectarinia_pca_*.recode.vcf ); do grep -v "^#" $i >> nectarinia.vcf; done


grep "#" sylvia_pca_25.recode.vcf > sylvia.vcf

for i in $( ls sylvia_pca_*.recode.vcf ); do grep -v "^#" $i >> sylvia.vcf; done


grep "#" turdus_pca_25.recode.vcf > turdus.vcf

for i in $( ls turdus_pca_*.recode.vcf ); do grep -v "^#" $i >> turdus.vcf; done


grep "#" zosterops_pca_25.recode.vcf > zosterops.vcf

for i in $( ls zosterops_pca_*.recode.vcf ); do grep -v "^#" $i >> zosterops.vcf; done


# remove individual scaffold files
rm *recode*
...
# convert  with plink
plink --file ${workdir}/05_pca/cossypha --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/cossypha_plink

# run pca on dataset
plink --file ${workdir}/05_pca/cossypha_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/cossypha_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/cristrio --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/cristrio_plink

# run pca on dataset
plink --file ${workdir}/05_pca/cristrio_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/cristrio_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/critrist --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/critrist_plink

# run pca on dataset
plink --file ${workdir}/05_pca/critrist_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/critrist_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/melaenornis --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/melaenornis_plink

# run pca on dataset
plink --file ${workdir}/05_pca/melaenornis_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/melaenornis_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/nectarinia --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/nectarinia_plink

# run pca on dataset
plink --file ${workdir}/05_pca/nectarinia_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/nectarinia_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/sylvia --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/sylvia_plink

# run pca on dataset
plink --file ${workdir}/05_pca/sylvia_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/sylvia_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/turdus --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/turdus_plink

# run pca on dataset
plink --file ${workdir}/05_pca/turdus_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/turdus_plink_pca
...
# convert  with plink
plink --file ${workdir}/05_pca/zosterops --recode12 --allow-extra-chr \
--out ${workdir}/05_pca/zosterops_plink

# run pca on dataset
plink --file ${workdir}/05_pca/zosterops_plink --pca --allow-extra-chr \
--out ${workdir}/05_pca/zosterops_plink_pca
