...
samtools view -b -S -o ${workdir}/01_bam_files/${basename_array}_unmerged.bam ${workdir}/01_bam_files/${basename_array}_unmerged.sam

# convert sam to bam for merged reads
samtools view -b -S -o ${workdir}/01_bam_files/${basename_array}_merged.bam ${workdir}/01_bam_files/${basename_array}_merged.sam
...
java -jar /home/jmanthey/picard.jar MarkDuplicates REMOVE_DUPLICATES=true MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=100 M=${workdir}/01_bam_files/${basename_array}_markdups_metric_file.txt I=${workdir}/01_bam_files/${basename_array}_cleaned_sorted_rg.bam O=${workdir}/01_bam_files/${basename_array}_prefinal.bam
